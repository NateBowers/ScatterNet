...
def gen_data_first(data,test_file,data_folder=None):
    ...
    print(train_X.all())
    max_val = np.amax(train_X)
    min_val = np.amin(train_X)
...
def design_spectrum(data,reuse_weights,output_folder,weight_name_save,weight_name_load,n_batch,numEpochs,lr_rate,lr_decay,num_layers,n_hidden,percent_val,patienceLimit,compare,sample_val,spect_to_sample,matchSpectrum,match_test_file,designSpectrum,design_test_file):
    ...
    X = tf.compat.v1.get_variable(name="b1", initializer=init_list_rand)
    ...
    # optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=lr_rate, decay=lr_decay).minimize(cost,var_list=[X])
    global_step = tf.Variable(0, trainable=False)

    learning_rate = tf.compat.v1.train.exponential_decay(lr_rate,global_step,1000,lr_decay, staircase=False)
    optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=learning_rate).minimize(cost, global_step=global_step, var_list=[X])
...
def match_spectrum(data,reuse_weights,output_folder,weight_name_save,weight_name_load,n_batch,numEpochs,lr_rate,lr_decay,num_layers,n_hidden,percent_val,patienceLimit,compare,sample_val,spect_to_sample,matchSpectrum,match_test_file,designSpectrum,design_test_file):
    ...
    for i in range(50):
        init_list_rand = tf.constant(np.random.rand(1,x_size)*40.0+30.0,dtype=tf.float32)
        X = tf.Variable(initial_value=init_list_rand, name="b1"+ str(i))
        y = tf.compat.v1.placeholder("float", shape=[None, y_size])
        # Forward propagation
        ...
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=lr_rate, beta2=lr_decay,epsilon=0.1).minimize(cost,var_list=[X])
        with tf.compat.v1.Session() as sess:
            init = tf.compat.v1.global_variables_initializer()
            sess.run(init)
            cum_loss = 0
            ...
            for step in range(numEpochs*10+1):
                ...
                if step%1000 == 0 or step == numEpochs*10:
                    print("Iteration " + str(i) + " : Step: " + str(step) + " : Loss: " + str(cum_loss) + " : " + str(X.eval()))
...
def main(data,reuse_weights,output_folder,weight_name_save,weight_name_load,n_batch,numEpochs,lr_rate,lr_decay,num_layers,n_hidden,match_test_file,design_test_file,matchSpectrum,designSpectrum,percent_val,patienceLimit=10,compare=False,sample_val=True,spect_to_sample=300):
    ...
    # This is to ensure similar results throughout. 
    RANDOM_SEED = 42
    ...
    print(lr_decay)
    print("--done--")
    learning_rate = tf.compat.v1.train.exponential_decay(lr_rate,global_step,int(train_X.shape[0]/n_batch),lr_decay,staircase=False)
    optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=learning_rate).minimize(cost,global_step=global_step)
        ...
        for curEpoch in range(numEpochs):
            ...
            if step ==  int(train_X.shape[0]/n_batch):            
                ...
                if (curEpoch % 10 == 0 or curEpoch == 1):
                    ...
                    print("Epoch: " + str(curEpoch) + " : Loss: " + str(cum_loss) + " : " + str(perinc))
...
if __name__=="__main__":
    ...
    parser.add_argument("--output_folder",type=str,default='results/8_layer_tio2') #Where to output the results to. Note: No / at the end. 
